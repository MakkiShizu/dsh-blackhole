...
上流チューナーと `ghostty-blackhole-main` の 15 の降着円盤プリセット、慣性付きの
ポインター追従、完全な設定ページ（設定 → **Blackhole**、ライブ状態：モード / レベル /
FPS / シェーダーエラー）、**星空背景**トグル。設定ページと本ドキュメントは
**简体中文 / 繁體中文 / 日本語 / English** の 4 言語に対応。下部の作業領域が覆われる
ことはなく、状態はすべてメモリ内のみです。
