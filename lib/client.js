...
`;
			...
			const lens = {
				target: null, prevFilter: null, svg: null, feImg: null, disp: null,
			...
			const ensureLens = () => {
				...
				if (!lens.svg) {
					...
					filter.setAttribute("id", "bh-lens-filter");
					...
					filter.appendChild(lens.feImg);
					filter.appendChild(lens.disp);
			...
			const regenLensMap = (drv, uTimeJS, tDrive, followW, fpx, fpy, cw, ch) => {
				const aspect = cw / ch;
				const hs = holeState(drv, uTimeJS, tDrive, aspect);
				if (!hs) { tearLens(); return; }
				const cx = hs.center[0] + (fpx - hs.center[0]) * followW;
				const cy = hs.center[1] + (fpy - hs.center[1]) * followW;
				const rh = hs.rh;
				const mw = 384;
				const mh = Math.max(64, Math.round(384 / aspect));
				if (mw !== lens.mw || mh !== lens.mh) {
					lens.mw = mw;
					lens.mh = mh;
					lens.map.width = mw;
					lens.map.height = mh;
				...
				for (let y = 0; y < mh; y++) {
					for (let x = 0; x < mw; x++) {
						...
						const Dx = mag * dirx * srcPerMap;
						const Dy = mag * diry * srcPerMap;
						const i = (y * mw + x) * 4;
						data[i] = Math.max(0, Math.min(255, Math.round(128 + (Dx / scaleAttr) * 255)));
						data[i + 1] = Math.max(0, Math.min(255, Math.round(128 + (Dy / scaleAttr) * 255)));
						data[i + 2] = 128;
						data[i + 3] = 255;
					}
				}
							...
							if (state.lensPage) {
								if (ensureLens()) {
									...
									if (!hsNow) {
										...
										const moved = Math.abs(cxNow - lens.lastCx) > 0.008 || Math.abs(cyNow - lens.lastCy) > 0.008 || Math.abs(hsNow.rh - lens.lastRh) > 0.003;
										if (lens.lastRegen < 0 || (moved && performance.now() - lens.lastRegen > 120)) {
											regenLensMap(drv, t, t * state.driftSpeed, followW, fpx, fpy, canvas.width, canvas.height);
										}
									}
								}
							} else if (lens.active) {
