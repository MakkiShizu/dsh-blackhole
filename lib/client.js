...
uniform float uSky; // 0 = no procedural starfield background (page shows through; the SVG lens warps it), 1 = lensed sky disc
uniform int   uSteps;
...
void main() {
    ...
    if (b >= bmax) {
        ...
        // far field: with the sky off nothing is drawn here — the SVG lens
        // warps the real page content instead
        vec3 col = uSky * (term + stars(d) * L.star * windowF);
        float alpha = uSky * clamp(uMasterOpacity * vis * shield * env, 0.0, 1.0);
    ...
    if (!captured) {
        vec3 d = normalize(v);
        bg += stars(d) * L.star * uSky * windowF * shield;
        if (d.z < -0.05) {
            ...
            bg += texture(uBackground, suv).rgb * toward * uSky;
        }
    }

    // sky on: opaque envelope over the lensed sky disc (original look).
    // sky off: alpha follows the hole content itself — the shadow (captured)
    // and the disk light stay, everything else is transparent so the (SVG
    // lensed) page shows through.
    vec3  diskCol = vec3(1.0) - exp(-emitc * L.expo);
    vec3  col     = bg * trans + diskCol;
    float envA    = clamp(uMasterOpacity * vis * shield * env, 0.0, 1.0);
    float coreA   = clamp(uMasterOpacity * vis * shield
        * (captured ? 1.0 : max(max(diskCol.r, diskCol.g), diskCol.b)), 0.0, 1.0);
    float alpha   = mix(coreA, envA, uSky);
    fragColor = vec4(col * alpha, alpha);
}
`;
			const UNIFORM_NAMES = [
			...
			// ---------- page-content lens: hole driver (mirrors the shader) ----------
			const smoothstepJS = (e0, e1, x) => {
				const t = Math.min(1, Math.max(0, (x - e0) / (e1 - e0)));
				return t * t * (3 - 2 * t);
			};
			const lissaJS = (tv) => [
				0.75 * Math.sin(tv * 0.37) + 0.25 * Math.sin(tv * 0.83 + 1.0),
				0.70 * Math.sin(tv * 0.54 + 2.1) + 0.30 * Math.sin(tv * 1.07),
			];
			// Same math the shader uses to place/size the hole (token & pomodoro
			// branches), so the SVG displacement map stays locked to the render.
			function holeState(drv, uTimeJS, tDrive, aspect) {
				const wa = state.workArea;
				const hr = state.holeRadius;
				if (drv.mode === 0) {
					const workSec = 55 * 60, cycleSec = 60 * 60;
					const phase = (Date.now() / 1000) % cycleSec;
					const collapse = Math.min(60, workSec * 0.15);
					let grow = Math.min(1, Math.max(0, phase / workSec)) * (1 - smoothstepJS(workSec - collapse, workSec, phase));
					let I = 0.12 + 0.88 * grow;
					const idle = Math.max(0, uTimeJS - rt.activityAt);
					I *= 1 - smoothstepJS(90, Math.max(300, 91), idle);
					const sz = 0.22 + 0.78 * I;
					const rh = hr * sz;
					const rout = Math.max(state.diskOuter, Math.max(state.diskInner, 1.6) + 0.5);
					const ext = (rout / 2.5980762) * rh;
					const yLo = wa + 0.12 + ext;
					const yHi = Math.max(yLo, 0.90 - ext);
					const spd = 0.35 + 0.65 * I;
					let cx = 0.5 + (0.24 * Math.sin(tDrive * 0.21) + 0.05 * Math.sin(tDrive * 0.083)) * spd;
					let cy = 1 - (yLo + (yHi - yLo) * (0.5 + (0.42 * Math.sin(tDrive * 0.157 + 2.0) + 0.08 * Math.sin(tDrive * 0.117)) * spd));
					cx += I * (0.040 * Math.sin(tDrive * 0.83) + 0.020 * Math.sin(tDrive * 1.31));
					cy += I * (0.030 * Math.sin(tDrive * 1.03 + 1.0));
					return { center: [cx, cy], rh };
				}
				let lvl;
				if (drv.mode === 3) lvl = 0.08 + 0.92 * Math.min(1, (uTimeJS % 42) / 40);
				else lvl = drv.level >= 0 ? drv.level : -1;
				if (lvl < 0) return null;
				const g = Math.pow(Math.min(1, Math.max(0, lvl)), state.tokenEase);
				const I = 0.10 + 0.90 * g;
				const rhMin = Math.sqrt((state.tokenAreaMin * aspect) / Math.PI);
				const rhMax = Math.sqrt((state.tokenAreaMax * aspect) / Math.PI);
				const rhT = (rhMin + (rhMax - rhMin) * g) * (hr / 0.08);
				const rh = rhT;
				const marg = Math.min(rhT * (1.45 + (0.90 - 1.45) * g), 0.5 * (1 - wa - 0.03));
				const xPad = marg / aspect;
				const fullLo = [Math.min(xPad, 0.5), marg];
				const fullHi = [Math.max(0.5, 1 - xPad), Math.max(marg, 1 - (wa + 0.03 + marg))];
				const corner = [
					Math.min(Math.max(state.tokenHomeX, fullLo[0]), fullHi[0]),
					Math.min(Math.max(state.tokenHomeY, fullLo[1]), fullHi[1]),
				];
				const reach = 0.06 + (Math.max(state.tokenReach, 0.06) - 0.06) * g;
				const lo = [corner[0] + (fullLo[0] - corner[0]) * reach, fullLo[1]];
				const hi = [fullHi[0], corner[1] + (fullHi[1] - corner[1]) * reach];
				const room = [Math.max(0, (hi[0] - lo[0]) * 0.5), Math.max(0, (hi[1] - lo[1]) * 0.5)];
				const wobAmp = [
					Math.min(0.010 + 0.030 * g, Math.max(room[0] * 0.35, 0.006)),
					Math.min(0.010 + 0.030 * g, Math.max(room[1] * 0.35, 0.006)),
				];
				const ampEff = [Math.max(0, room[0] - wobAmp[0]), Math.max(0, room[1] - wobAmp[1])];
				const calm = lissaJS(tDrive * state.tokenCalm);
				const rush = lissaJS(tDrive * state.tokenRush);
				const wander = [calm[0] + (rush[0] - calm[0]) * g, calm[1] + (rush[1] - calm[1]) * g];
				const cx = (lo[0] + hi[0]) * 0.5 + wander[0] * ampEff[0] + wobAmp[0] * Math.cos(tDrive * 0.8);
				const cy = (lo[1] + hi[1]) * 0.5 + wander[1] * ampEff[1] + wobAmp[1] * Math.sin(tDrive * 1.0);
				return { center: [cx, cy], rh };
			}

			// ---------- page-content lens: SVG feDisplacementMap over the center column ----------
			const SVG_NS = "http://www.w3.org/2000/svg";
			const lens = {
				target: null, prevFilter: null, svg: null, filter: null, feImg: null, disp: null,
				map: null, mctx: null, mw: 0, mh: 0, active: false,
				lastRegen: -1, lastCx: 0, lastCy: 0, lastRh: 0,
			};
			const clampByte = (v) => (v < 0 ? 0 : v > 255 ? 255 : Math.round(v));
			const findLensTarget = () => {
				const el = document.querySelector('#root [class$="_centerCol"]');
				return el && el.getBoundingClientRect().width > 0 ? el : null;
			};
			const ensureLens = () => {
				if (lens.active && lens.target) return true;
				const target = findLensTarget();
				if (!target) return false;
				if (!lens.svg) {
					lens.svg = document.createElementNS(SVG_NS, "svg");
					lens.svg.setAttribute("width", "0");
					lens.svg.setAttribute("height", "0");
					lens.svg.setAttribute("style", "position:absolute;width:0;height:0");
					const defs = document.createElementNS(SVG_NS, "defs");
					lens.filter = document.createElementNS(SVG_NS, "filter");
					lens.filter.setAttribute("id", "bh-lens-filter");
					// Region starts as the full element box; regenLensMap narrows it
					// to a square around the hole (bounds the filter cost).
					lens.filter.setAttribute("x", "0");
					lens.filter.setAttribute("y", "0");
					lens.filter.setAttribute("width", "100%");
					lens.filter.setAttribute("height", "100%");
					lens.feImg = document.createElementNS(SVG_NS, "feImage");
					lens.feImg.setAttribute("result", "dm");
					lens.feImg.setAttribute("preserveAspectRatio", "none");
					lens.feImg.setAttribute("x", "0");
					lens.feImg.setAttribute("y", "0");
					lens.feImg.setAttribute("width", "100%");
					lens.feImg.setAttribute("height", "100%");
					lens.disp = document.createElementNS(SVG_NS, "feDisplacementMap");
					lens.disp.setAttribute("in", "SourceGraphic");
					lens.disp.setAttribute("in2", "dm");
					lens.disp.setAttribute("scale", "120");
					lens.disp.setAttribute("xChannelSelector", "R");
					lens.disp.setAttribute("yChannelSelector", "G");
					lens.filter.appendChild(lens.feImg);
					lens.filter.appendChild(lens.disp);
					defs.appendChild(lens.filter);
					lens.svg.appendChild(defs);
					document.body.appendChild(lens.svg);
					lens.map = document.createElement("canvas");
					lens.mctx = lens.map.getContext("2d");
				}
				lens.target = target;
				lens.prevFilter = target.style.filter;
				target.style.filter = "url(#bh-lens-filter)";
				lens.active = true;
				return true;
			};
			const tearLens = () => {
				if (lens.target && lens.active) lens.target.style.filter = lens.prevFilter || "";
				lens.active = false;
				lens.target = null;
			};
			// Regenerate the displacement map (R/G channels encode dx/dy). The
			// filter region narrows to a square around the hole (bounds the
			// per-repaint filter cost), and the map is generated for exactly that
			// region, so the warp stays locked to the hole. The feDisplacementMap
			// scale attribute multiplies, so lensStrength updates need no regen.
			const regenLensMap = (drv, uTimeJS, tDrive, followW, fpx, fpy) => {
				if (!lens.target) return;
				const rect = lens.target.getBoundingClientRect();
				const elemW = Math.max(1, rect.width);
				const elemH = Math.max(1, rect.height);
				const hs = holeState(drv, uTimeJS, tDrive, elemW / elemH);
				if (!hs) { tearLens(); return; }
				const cx = hs.center[0] + (fpx - hs.center[0]) * followW;
				const cy = hs.center[1] + (fpy - hs.center[1]) * followW;
				const rh = hs.rh;
				const halfPx = Math.max(40, Math.min(0.55 * Math.min(elemW, elemH), rh * elemH * 5));
				const cxPx = cx * elemW, cyPx = cy * elemH;
				const x0 = Math.max(0, cxPx - halfPx), y0 = Math.max(0, cyPx - halfPx);
				const x1 = Math.min(elemW, cxPx + halfPx), y1 = Math.min(elemH, cyPx + halfPx);
				const rw = Math.max(1, x1 - x0), rhh = Math.max(1, y1 - y0);
				lens.filter.setAttribute("x", ((x0 / elemW) * 100).toFixed(2) + "%");
				lens.filter.setAttribute("y", ((y0 / elemH) * 100).toFixed(2) + "%");
				lens.filter.setAttribute("width", ((rw / elemW) * 100).toFixed(2) + "%");
				lens.filter.setAttribute("height", ((rhh / elemH) * 100).toFixed(2) + "%");
				const mw = 256, mh = 256;
				if (lens.mw !== mw || lens.mh !== mh) {
					lens.mw = mw;
					lens.mh = mh;
					lens.map.width = mw;
					lens.map.height = mh;
				}
				const img = lens.mctx.createImageData(mw, mh);
				const data = img.data;
				const px = ((cxPx - x0) / rw) * mw;
				const py = ((cyPx - y0) / rhh) * mh;
				const bcrit = rh * elemH * (mw / rw); // shadow radius in map texels
				const scaleAttr = 120;
				const sxp = rw / mw, syp = rhh / mh; // element px per texel
				for (let y = 0; y < mh; y++) {
					for (let x = 0; x < mw; x++) {
						const dx = x - px, dy = y - py;
						const r = Math.sqrt(dx * dx + dy * dy);
						let mag = 0, dirx = 0, diry = 0;
						if (r > 0.5 && r < bcrit * 5) {
							if (r < bcrit * 0.98) {
								mag = (bcrit - r) * 1.0;
								dirx = dx / r;
								...
								mag = (bcrit * bcrit) / r * 1.4;
								dirx = -dx / r;
								diry = -dy / r;
							}
							mag *= Math.max(0, 1 - r / (bcrit * 5));
						}
						const Dx = mag * dirx * sxp;
						const Dy = mag * diry * syp;
						const i = (y * mw + x) * 4;
						data[i] = clampByte(128 + (Dx / scaleAttr) * 255);
						data[i + 1] = clampByte(128 + (Dy / scaleAttr) * 255);
						data[i + 2] = 128;
						data[i + 3] = 255;
					}
				}
				lens.mctx.putImageData(img, 0, 0);
				lens.feImg.setAttribute("href", lens.map.toDataURL("image/png"));
				lens.lastCx = cx;
				lens.lastCy = cy;
				lens.lastRh = rh;
				lens.lastRegen = performance.now();
			};
				...
				React.useEffect(() => {
					...
					try {
						...
						const resize = () => {
							...
							lens.lastRegen = -1; // aspect changed: rebuild the displacement map
						};
						resize();
						...
						const frame = (ms) => {
							...
							const fpy = Math.min(Math.max(rt.pointerY - 0.1, 0.04), Math.max(0.08, state.workArea - 0.03));
							...
							gl.uniform2f(U.uFollowPos, fpx, fpy);
							...
							gl.uniform1f(U.uMasterOpacity, state.masterOpacity);
							gl.uniform1f(U.uSky, state.sky ? 1 : 0);
							gl.uniform1i(U.uSteps, state.quality);
							gl.drawArrays(gl.TRIANGLES, 0, 3);

							// page-content lens: SVG feDisplacementMap over the center
							// column, displacement map regenerated only when the hole
							// moves/grows (throttled ~8 Hz); strength rides the scale attr
							if (state.lensPage) {
								if (ensureLens()) {
									lens.disp.setAttribute("scale", String(120 * state.lensStrength));
									const aspect = canvas.width / canvas.height;
									const hsNow = holeState(drv, t, t * state.driftSpeed, aspect);
									if (!hsNow) {
										tearLens();
									} else {
										const cxNow = hsNow.center[0] + (fpx - hsNow.center[0]) * followW;
										const cyNow = hsNow.center[1] + (fpy - hsNow.center[1]) * followW;
										const moved = Math.abs(cxNow - lens.lastCx) > 0.012 || Math.abs(cyNow - lens.lastCy) > 0.012 || Math.abs(hsNow.rh - lens.lastRh) > 0.004;
										if (lens.lastRegen < 0 || (moved && performance.now() - lens.lastRegen > 150)) {
											regenLensMap(drv, t, t * state.driftSpeed, followW, fpx, fpy);
										}
									}
								}
							} else if (lens.active) {
					...
					return () => {
						...
						tearLens();
						if (lens.svg) { try { lens.svg.remove(); } catch (e) { /* noop */ } }
						lens.svg = null;
			...
			function SettingsPage(props) {
				...
				const slider = (key, label, min, max, step, fmt) => row(
					label,
					h("input", { type: "range", min, max, step, value: s[key], onChange: onNum(key), style: S.range }),
					h("span", { style: S.val }, fmt ? fmt(s[key]) : String(s[key])),
				);
				const group = (title, children) => h("div", { style: S.group, key: title },
					h("p", { style: S.groupTitle }, title), ...children);
				const pct = (v) => Math.round(v * 100) + " %";
				const MODE_LABELS = { demo: "Demo tour", pomodoro: "Pomodoro", context: "Context", manual: "Manual", off: "Off" };

				const modeSelect = h("select", { value: s.mode, onChange: onStr("mode"), style: S.input },
					h("option", { value: "demo" }, "Demo tour — 42 s preset loop"),
					h("option", { value: "pomodoro" }, "Pomodoro — 55/5 wall clock"),
					h("option", { value: "context" }, "Context — conversation size"),
					h("option", { value: "manual" }, "Manual — fixed level"),
				...
				const qualitySelect = h("select", { value: String(s.quality), onChange: (ev) => patch({ quality: parseInt(ev.target.value, 10) }), style: S.input },
					h("option", { value: "24" }, "Low — 24 geodesic steps"),
					h("option", { value: "48" }, "Medium — 48 steps"),
					h("option", { value: "72" }, "High — 72 steps"));
				...
				if (rt.statusKind === "error") stText = "✗ " + rt.statusText;
					...
					else if (s.mode === "context") lvlTxt = rt.ctxNodes < 0 ? "hidden (no session)" : Math.round((1 - Math.exp(-rt.ctxNodes / 70)) * 100) + " %";
					else if (s.mode === "pomodoro") lvlTxt = "wall clock";
					stText = "running · " + (MODE_LABELS[s.mode] || s.mode) + " · " + lvlTxt + " · " + rt.fps + " fps";
				...
				if (s.mode === "context") {
					...
					modeNote = h("p", { style: S.note },
						"No session open or the session is blank → no hole. Conversation nodes: " + (n == null || n < 0 ? "—" : n) +
						" → fill ≈ " + Math.round(fill * 100) + " %. A fresh session seeds a tiny hole in the top-right corner.");
				} else if (s.mode === "pomodoro") {
					modeNote = h("p", { style: S.note }, "Grows through 55 min of work, stays small through the 5 min break, and shrinks after 90 s without keyboard/pointer activity.");
				} else if (s.mode === "demo") {
					modeNote = h("p", { style: S.note }, "Self-running showcase: grows from a visible seed to full size while touring the tuner presets (Inferno → Gargantua → M87* → Ember → Quasar → Blazar → Pure Lens).");
				} else if (s.mode === "off") {
					modeNote = h("p", { style: S.note }, "The effect is hidden; the overlay is fully transparent.");
				}

				return h("div", { style: S.page },
					h("h3", { style: S.title }, "Blackhole"),
					h("p", { style: S.sub }, "Geodesic-traced Schwarzschild black hole, ported from s0xDk/ghostty-blackhole. It floats above the page; the bottom work area stays clear."),
					h("div", { style: S.statusRow },
						h("span", { style: Object.assign({}, S.statusDot, { background: stColor }) }),
						h("span", { style: S.statusText }, stText)),
					group("Drive", [
						row("Mode", modeSelect, null),
						s.mode === "manual" ? slider("level", "Manual level", 0, 1, 0.01, pct) : null,
						modeNote,
					].filter(Boolean)),
					group("Preset", [row("Disk look", presetSelect, null)]),
					group("Size & motion", [
						slider("holeRadius", "Hole radius", 0.005, 0.15, 0.005, (v) => v.toFixed(3)),
						slider("tokenAreaMax", "Max fill area", 0.02, 1, 0.01, pct),
						slider("lensDepth", "Lens depth", 4, 40, 1, (v) => Math.round(v)),
						slider("driftSpeed", "Drift speed", 0, 3, 0.05, (v) => v.toFixed(2)),
						slider("workArea", "Work area (bottom)", 0.05, 0.7, 0.01, pct),
						row("Follow pointer", h("input", { type: "checkbox", checked: s.followMouse, onChange: onBool("followMouse") }), null),
					]),
					group("Disk", [
						slider("diskTemp", "Disk temperature", 2000, 25000, 500, (v) => Math.round(v) + " K"),
						slider("diskGain", "Disk gain", 0, 5, 0.1, (v) => v.toFixed(1)),
						slider("diskOpacity", "Disk opacity", 0, 1, 0.05, (v) => v.toFixed(2)),
						slider("diskContrast", "Streak contrast", 0, 3, 0.1, (v) => v.toFixed(1)),
						slider("starGain", "Starfield gain", 0, 1.5, 0.05, (v) => v.toFixed(2)),
						slider("exposure", "Exposure", 0.2, 3, 0.05, (v) => v.toFixed(2)),
						slider("dilationMin", "Time dilation floor", 0.05, 1, 0.05, (v) => v.toFixed(2)),
					]),
					group("Background & lensing", [
						row("Sky background", h("input", { type: "checkbox", checked: s.sky, onChange: onBool("sky") }), null),
						row("Lens page", h("input", { type: "checkbox", checked: s.lensPage, onChange: onBool("lensPage") }), null),
						slider("lensStrength", "Lens strength", 0, 2, 0.05, (v) => v.toFixed(2)),
						h("p", { style: S.note }, "Sky off + Lens on (default): the real page content is warped around the hole by an SVG displacement filter on the conversation column. Sky on: the procedural starfield disc returns and the page lens has nothing to show through."),
					]),
					group("Render", [
						row("Quality", qualitySelect, null),
						slider("masterOpacity", "Opacity", 0, 1, 0.05, pct),
						h("button", { style: S.btn, type: "button", onClick: () => patch(Object.assign({}, DEFAULTS)) }, "Reset defaults"),
					]),
					h("p", { style: S.note }, "Persistent web plugin (dsh-blackhole). State lives in memory only — nothing is persisted. Credits: s0xDk/ghostty-blackhole (MIT), XboxNahida/ghostty-blackhole-main, 962412311/win-ghostty-blackhole; physics after Eric Bruneton’s black hole shader."),
				);
			}
			...
			slots.inject("settings.section", () => slots.register(
				{ name: "settings.section", id: "blackhole", order: 30, label: "Blackhole" },
