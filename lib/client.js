	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const rt = {
				...
				// displayed disk look (uniform source) vs the glide target
				look: {
					diskTemp: state.diskTemp, diskIncl: state.diskIncl, diskRoll: state.diskRoll,
					diskInner: state.diskInner, diskOuter: state.diskOuter, diskOpacity: state.diskOpacity,
					dopplerMix: state.dopplerMix, diskBeam: state.diskBeam, diskGain: state.diskGain,
					diskContrast: state.diskContrast, diskWind: state.diskWind, diskSpeed: state.diskSpeed,
					exposure: state.exposure, starGain: state.starGain,
				},
				lookTarget: null,
			};
			// Read-only diagnostics for the look glide (support/debug).
			window.__bhLook = {
				get look() { return rt.look; },
				get target() { return rt.lookTarget; },
			};
			const LOOK_KEYS = ["diskTemp", "diskIncl", "diskRoll", "diskInner", "diskOuter", "diskOpacity",
				"dopplerMix", "diskBeam", "diskGain", "diskContrast", "diskWind", "diskSpeed", "exposure", "starGain"];
			const defaultLook = () => {
				const t = {};
				for (const k of LOOK_KEYS) t[k] = DEFAULTS[k];
				return t;
			};
			// Smooth transition: the target lands instantly (UI shows it), the
			// rendered look glides toward it over ~1.5 s (exponential, tau 0.6 s).
			const startLookGlide = (target) => { rt.lookTarget = target; };
			...
			const applyAutoLook = () => {
				...
				if (state.autoMode === "preset") {
					...
					target = Object.assign({ preset: k }, PRESETS[k]);
				} else {
...
`;
				...
				React.useEffect(() => {
					...
					try {
						...
						const frame = (ms) => {
							...
							// glide the displayed disk look toward the target
							if (rt.lookTarget) {
								let done = true;
								for (const k of LOOK_KEYS) {
									const diff = rt.lookTarget[k] - rt.look[k];
									if (Math.abs(diff) > 1e-4) {
										done = false;
										rt.look[k] += diff * (1 - Math.exp(-dt / 0.6));
									}
								}
								if (done) {
									rt.look = Object.assign({}, rt.lookTarget);
									rt.lookTarget = null;
								}
							}
							gl.uniform1f(U.uStarGain, rt.look.starGain);
							gl.uniform1f(U.uDiskInner, rt.look.diskInner);
							gl.uniform1f(U.uDiskOuter, rt.look.diskOuter);
							gl.uniform1f(U.uDiskIncl, rt.look.diskIncl);
							gl.uniform1f(U.uDiskRoll, rt.look.diskRoll);
							gl.uniform1f(U.uDiskGain, rt.look.diskGain);
							gl.uniform1f(U.uDiskOpacity, rt.look.diskOpacity);
							gl.uniform1f(U.uDiskTemp, rt.look.diskTemp);
							gl.uniform1f(U.uDopplerMix, rt.look.dopplerMix);
							gl.uniform1f(U.uDiskBeam, rt.look.diskBeam);
							gl.uniform1f(U.uDiskSpeed, rt.look.diskSpeed);
							gl.uniform1f(U.uDiskWind, rt.look.diskWind);
							gl.uniform1f(U.uDiskContrast, rt.look.diskContrast);
							gl.uniform1f(U.uExposure, rt.look.exposure);
			...
			function SettingsPage(props) {
				...
				const onNum = (key) => (ev) => {
					...
					patch({ [key]: v });
					if (LOOK_KEYS.includes(key)) {
						rt.look[key] = v;               // direct control wins instantly
						if (rt.lookTarget) rt.lookTarget[key] = v; // and cancels the glide on that field
					}
				};
				...
				const presetSelect = h("select", { value: s.preset, onChange: (ev) => {
					...
					if (p) {
						const t = Object.assign({ preset: ev.target.value }, p);
						patch(t);
						startLookGlide(t);
					}
				}, style: S.input },
				...
				return h("div", { style: S.page },
					...
					group(L.gRender, [
						...
						h("button", { style: S.btn, type: "button", onClick: () => {
							patch(Object.assign({}, DEFAULTS));
							startLookGlide(defaultLook());
						} }, L.reset),
					]),
