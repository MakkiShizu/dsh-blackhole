...
uniform float uSky; // 1 = lensed starfield sky is drawn around the hole; 0 = page shows through (plain hole / element-transform lens)
uniform int   uSteps;
...
void main() {
    ...
    // and the disk light stay, everything else is transparent so the untouched
    // page shows through (the element-transform lens warps it).
...
`;
			...
			// ---------- page-content lens: per-element CSS transforms ----------
			// Filter-free, snapshot-free content lensing: DOM elements around the
			// hole get a small radial pull toward it plus magnification, applied
			// as transforms only (layout untouched — no SVG filter region
			// clipping, no canvas taint, nothing ever goes black). The shadow and
			// disk stay on the WebGL overlay above. The candidate list is
			// refreshed when the hole moves (throttled); per-update math uses
			// stored layout rects so transforms cannot feed back into the measure.
			const domLens = {
				col: null, candidates: [], lastUpdate: -1, lastRefresh: -1,
				base: [], applied: new Map(),
			};
			// Read-only diagnostics for the element-transform lens.
			window.__bhLens = {
				get hs() { return domLens.lastHs; },
				get candidates() { return domLens.candidates.length; },
				get applied() { return domLens.applied.size; },
				get lastRefresh() { return domLens.lastRefresh; },
			};
			const findContentCol = () => {
				const el = document.querySelector('#root [class$="_centerCol"]');
				return el && el.getBoundingClientRect().width > 0 ? el : null;
			};
			const lensStrengthScale = () => 0.18 + 0.55 * state.lensStrength;
			// Re-scan the column for elements intersecting the lens circle. Pick
			// the SMALLEST intersecting blocks first (leaf-ish: spans, buttons,
			// cards) and skip any element that already has an included ancestor,
			// so whole-column containers are never transformed as a unit.
			const refreshLensCandidates = () => {
				const col = findContentCol();
				domLens.col = col;
				domLens.candidates = [];
				domLens.base = [];
				if (!col) return;
				const hs = domLens.lastHs;
				if (!hs) return;
				const R = hs.rh * window.innerHeight * 4 + 80;
				...
				for (const el of col.querySelectorAll("*")) {
					...
					if (r.width > colRect.width * 0.85 && r.height > colRect.height * 0.5) continue;
					if (r.right < cx - R || r.left > cx + R || r.bottom < cy - R || r.top > cy + R) continue;
					set.add(el);
				}
				...
				for (const el of sorted) {
					if (domLens.candidates.length >= 40) break;
					let ancestorAdded = false;
					let p = el.parentElement;
					while (p && p !== col) {
						if (added.has(p)) { ancestorAdded = true; break; }
						p = p.parentElement;
					}
					if (ancestorAdded) continue;
					const r = el.getBoundingClientRect();
					domLens.candidates.push(el);
					domLens.base.push({ x: r.left + r.width / 2, y: r.top + r.height / 2, w: r.width, h: r.height });
					added.add(el);
				}
			};
			// Apply/clear transforms from the stored base rects (no feedback).
			const applyLensTransforms = () => {
				const hs = domLens.lastHs;
				if (!hs) return;
				const R = hs.rh * window.innerHeight * 4 + 80;
				const cx = hs.cx, cy = hs.cy;
				const k = lensStrengthScale();
				for (let i = 0; i < domLens.candidates.length; i++) {
					const el = domLens.candidates[i];
					const b = domLens.base[i];
					...
					if (r < R && r > 1) {
						const t = 1 - r / R;
						const tt = t * t;
						const mag = tt * 0.30 * R * k;
						const m = 1 + tt * 0.22 * k;
						const udx = (dx / r) * mag;
						const udy = (dy / r) * mag;
						const prev = domLens.applied.get(el);
						if (!prev) domLens.applied.set(el, { transform: el.style.transform, zIndex: el.style.zIndex });
						el.style.transform = "translate(" + udx.toFixed(2) + "px," + udy.toFixed(2) + "px) scale(" + m.toFixed(4) + ")";
						el.style.zIndex = "8";
					} else if (domLens.applied.has(el)) {
						const prev = domLens.applied.get(el);
						el.style.transform = prev.transform;
						el.style.zIndex = prev.zIndex;
						domLens.applied.delete(el);
					}
				}
			};
			const clearLensTransforms = () => {
				for (const [el, prev] of domLens.applied) {
					try {
						el.style.transform = prev.transform;
						el.style.zIndex = prev.zIndex;
					} catch (e) { /* noop */ }
				}
				domLens.applied.clear();
				domLens.candidates = [];
				domLens.base = [];
			};
				...
				React.useEffect(() => {
					...
					try {
						...
						const resize = () => {
							...
							domLens.lastRefresh = -1; // viewport changed: rescan
						};
						resize();
						...
						const frame = (ms) => {
							...
							// page-content lens: per-element CSS transforms around
							// the hole (transform-only — the live page is never
							// filtered, so nothing clips and repaints stay cheap).
							{
								const aspect = canvas.width / canvas.height;
								const hsNow = holeState(drv, t, t * state.driftSpeed, aspect);
								const wantLens = state.lensPage && !state.sky && hsNow;
								if (wantLens) {
									// convert the hole to VIEWPORT PIXELS — the candidate
									// scan and the transform math work in px
									domLens.lastHs = {
										cx: (hsNow.center[0] + (fpx - hsNow.center[0]) * followW) * window.innerWidth,
										cy: (hsNow.center[1] + (fpy - hsNow.center[1]) * followW) * window.innerHeight,
										rh: hsNow.rh,
									};
									if (domLens.lastRefresh < 0 || performance.now() - domLens.lastRefresh > 450) {
										domLens.lastRefresh = performance.now();
										refreshLensCandidates();
									}
									const nowMs = performance.now();
									if (nowMs - domLens.lastUpdate > 80) {
										domLens.lastUpdate = nowMs;
										applyLensTransforms();
									}
								} else {
									domLens.lastHs = null;
			...
			function SettingsPage(props) {
				...
				return h("div", { style: S.page },
					...
					group("Background & lensing", [
						...
						h("p", { style: S.note }, "Sky off + Lens on (default): the real DOM elements around the hole are warped by per-element transforms (magnified + pulled toward the hole) — no page-wide filter, so scrolling stays fast and nothing ever goes black. Sky on: the procedural starfield disc returns."),
					]),
					group("Render", [
