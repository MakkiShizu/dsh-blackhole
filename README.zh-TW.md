...
上游調音器與 `ghostty-blackhole-main` 的 15 個吸積盤預設、帶慣性的跟隨指標模式
（位置緩動）、完整的設定頁（設定 → **Blackhole**，含即時狀態：模式 / 等級 / FPS /
著色器錯誤）、**星空背景**開關，以及獨立的黑洞/星空不透明度滑桿。設定頁與本文檔支援
**简体中文 / 繁體中文 / 日本語 / English** 四種語言。底部工作區永不被覆蓋；
所有狀態僅存於記憶體。
