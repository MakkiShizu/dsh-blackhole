...
上游调音器与 `ghostty-blackhole-main` 的 15 个吸积盘预设、带惯性的跟随指针模式
（位置缓动）、完整的设置页（设置 → **Blackhole**，含实时状态：模式 / 等级 / FPS /
着色器错误）、**星空背景**开关，以及独立的黑洞/星空不透明度滑块。设置页与本文档支持
**简体中文 / 繁體中文 / 日本語 / English** 四种语言。底部工作区永不被覆盖；
所有状态仅存于内存。
